...
public:
    ...
    // R U L E  O F  5
    /*
    Typically the default implementations of these are sufficient, it rare cases you will need to implement them.
    */
    ~Matrix() noexcept; // deconstructor
    Matrix(const Matrix<T> &copy); // copy constructor
    Matrix<T> &operator=(const Matrix<T> &copy);// copy assignment operator
    // && -> typically means rvalue
    Matrix(const Matrix<T> &&copy) noexcept;// move constructor
    Matrix<T> &operator=(const Matrix<T> &&copy) noexcept; // move assignment operator
...
template <typename T>
Matrix<T>::Matrix(const Matrix<T> &copy): a(copy.get_a()), b(copy.get_b()), c(copy.get_c()), d(copy.get_d()) {
...
template <typename T>
Matrix<T>::Matrix(const Matrix<T> &&copy) noexcept
    : a(std::move(copy.get_a())), b(std::move(copy.get_b())), c(std::move(copy.get_c())), d(std::move(copy.get_d())) {

    std::cout << "Move constructor" << std::endl;
}

template <typename T>
Matrix<T> &Matrix<T>::operator=(const Matrix<T> &&copy) noexcept {
    if (this != &copy) {
        set_a(std::move(copy.get_a()));
        set_b(std::move(copy.get_b()));
        set_c(std::move(copy.get_c()));
        set_d(std::move(copy.get_d()));
    }
